...
```
Another common visualization of sound files are Spectrograms. @Smruti, please ad information what spectrograms are and why they are important/ what they tell us. Then plese explean the steps of the code.
also I am getting a "data set ‘sig1’ not founddata set ‘sig2’ not founddata set ‘sig3’ not founddata set ‘sig4’ not founddata set ‘sig5’ not found" error, are you seeing that too? Do we need to call data(sigX) every time? It seems to run fine for me without it
```{r}
par(mfcol=c(3,2))
...
Dominant frequency (DF) is a  useful characteristic of species calls. It is defined as the frequency of sound with the highest amplitude in our sample at each time point. We can easily find the DF of our samples using the dfreq function in the seewave package. In order to get the dominant frequency across the entire recording, we need to set the window length in dfreq to the lenghtof each signal.
```{r}
#Calculating Dominant frequency across entire signals
df1 = dfreq(dat1,wl = length(sig1))[2]
df2 = dfreq(dat2,wl = length(sig2))[2]
df3 = dfreq(dat3,wl = length(sig3))[2]
df4 = dfreq(dat4,wl = length(sig4))[2]
df5 = dfreq(dat5,wl = length(sig5))[2]
